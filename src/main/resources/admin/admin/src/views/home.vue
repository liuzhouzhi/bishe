...
<div class="content" :style='{"minHeight":"calc(100vh - 200px)","padding":"0px 30px 30px","margin":"0 auto","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230831/c5c18c4ebc614a97b159eb53953cee14.png) no-repeat center top / cover,#f5f5f5","display":"flex","width":"calc(100% - 60px)","fontSize":"16px","height":"100%"}'>
	...
	<div :style='{"padding":"0","boxShadow":"0 0px 0px 0px rgba(100,100,100,.05)","margin":"0 0px 30px 0px","borderColor":"#fceaee","alignItems":"center","color":"#666","display":"flex","justifyContent":"center","borderRadius":"3px","flexWrap":"wrap","background":"none","borderWidth":"0px","width":"100%","fontSize":"14px","borderStyle":"double","order":"1"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"0 10px 0 0","margin":"0 0 10px 0","borderColor":"#f0f0f0","borderRadius":"3px","textAlign":"center","background":"rgba(255, 255, 255,1)","borderWidth":"1px","display":"flex","width":"20%","borderStyle":"solid","justifyContent":"center"}' v-if="isAuth('ziyuanxinxi','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"#2aca76","fontWeight":"bold","height":"24px"}'>{{ziyuanxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"#2aca76","height":"24px"}'>资源信息总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<!-- echarts -->
	<!-- 1 -->
	<div class="type1" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","justifyContent":"flex-start","height":"auto","order":"10"}'>
		<div id="ziyuanxinxiChart1" class="echarts1" v-if="isAuth('ziyuanxinxi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getziyuanxinxiCount();
		this.ziyuanxinxiChat1();
	},
	methods:{
		// 词云
		...
		getziyuanxinxiCount() {
			this.$http({
				url: `ziyuanxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.ziyuanxinxiCount = data.data
				}
			})
		},
		ziyuanxinxiChat1() {
        ...
        var ziyuanxinxiChart1 = echarts.init(document.getElementById("ziyuanxinxiChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                ziyuanxinxiChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    ziyuanxinxiChart1.resize();
                };
            }
        });
      })
